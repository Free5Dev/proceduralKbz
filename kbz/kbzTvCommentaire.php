<?php 
	...
	if(isset($_GET['ref']))
	{
		$reqKbzTv=$bdd->prepare("SELECT k.id,k.titre,k.url,k.invite,k.realisateur,k.description,date_format(dateTv, '%d/%m/%Y à %Hh%imin%ss') as dateTv,k.url,m.nom FROM kbztv as k, membre as m where k.idMembre=m.id and k.id=?");
		$reqKbzTv->execute(array($_GET['ref']));
		$donneesKbzTv=$reqKbzTv->fetch();
	}
	else
	{
		header("Location:kbzTv.php");
	}
				...
				<div class="row">
					<div class="col-md-9  col-sm-12 kbzTvGauche">
						<div class="kbzTvGaucheContenu">
							...
							<p><iframe style="width: 900px;height: 500px;" class="img-fluid border border-dark"  src="<?php echo $donneesKbzTv['url']; ?>" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe></p>
							...
							<div id="commentaire">
								...
								<div class="fb-comments" data-href="http://localhost/phpProcedural/kbz/kbzTvCommentaire.php?ref=<?php echo $donneesKbzTv['id']; ?>" data-width="100%" data-numposts="5"></div>
							</div>
					...
					<div class="col-md-3 col-sm-12 kbzTvDroite" style="padding: 0px;">
						<div class="kbzTvDroiteContenu" >
							...
							<p><a href="index.php"><iframe width="350" height="190" src="<?php echo $donneesBlog['url']; ?>" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe></a></p>
